...
## Execution Flow
1.  **Environment Initialization:** Sets expansion configurations (`PYTORCH_CUDA_ALLOC_CONF`), injects API tokens, and dynamically rewrites `infer_pipeline.json` with target dataset paths.
2.  **Staging & Isolation:** Verifies remote mounts and migrates model classes and `best.pt` checkpoint weights to local block storage.
3.  **Stateful Inference Loop:** Scans target directories, bypasses existing processed frames, and slides a 128x128 window across Maxar satellite imagery to extract 2048-dimensional feature embeddings.
4.  **Cosine Similarity Matching:** Ranks similarity scores between ground chips and satellite frames, applying the spatial aggregation algorithm to determine final coordinates.
5.  **Audit & Package:** Verifies the final prediction count against raw input file totals, constructs the nested Codabench directory tree, and compiles the final submission `.zip`.
